...
**WoolSculptor** 是一个纯本地运行的 Minecraft AI 建筑代理（Agent）。它能够监听游戏内的玩家指令，通过自然语言（**仅限英文**）在本地实时生成 3D 模型，并将其自动转化为体素（Voxel），最终使用羊毛方块在你的面前将它建造出来。

完全开源，**零 API 调用成本**，让你的显卡在 Minecraft 世界里真正“无中生有”。

## 💡 核心特性

* **纯本地推理**：基于 OpenAI 的 `Shap-E` 模型生成 3D 点云，无需连接任何收费 API，方便快速还安全！
* **实时空间离散化**：使用 `Open3D` 进行高效的连续空间到离散网格（体素化）的矩阵计算，好算法值得信赖。
* **智能颜色匹配**：将 3D 模型的表面颜色映射到最接近的 Minecraft 羊毛方块数据值。
...
### 1. 环境准备
本项目需要你的电脑拥有一张支持 CUDA 的 Nvidia 显卡（推荐 RTX 4060 及以上）以获取最佳体验。
```bash
...
### 2. ⚠️ 模型权重下载 (必看)
本项目依赖 OpenAI 的 Shap-E 视觉大模型。当你**第一次**运行代码时，程序会在后台自动下载模型权重（总计约 1.5 GB）。
如果你的网络不稳定，经常遇到 `IncompleteRead` 或断流报错，**强烈建议通过浏览器或下载工具手动下载**以下两个文件：
* [transmitter.pt](https://openaipublic.azureedge.net/main/shap-e/transmitter.pt)
* [text300M.pt](https://openaipublic.azureedge.net/main/shap-e/text300M.pt)

下载完成后，请将它们放入系统的 Shap-E 缓存文件夹中：
...
### 2. ⚠️ Model Weights (Important)
This project relies on OpenAI's Shap-E model. When you run the code for the **first time**, it will automatically download the model weights (approx. 1.5 GB) in the background.
If you have an unstable network connection and encounter `IncompleteRead` or timeout errors, it is **highly recommended to download them manually**:
* [transmitter.pt](https://openaipublic.azureedge.net/main/shap-e/transmitter.pt)
* [text300M.pt](https://openaipublic.azureedge.net/main/shap-e/text300M.pt)

Once downloaded, place them in your system's Shap-E cache directory:
